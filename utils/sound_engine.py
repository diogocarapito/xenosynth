...
def audio_callback(outdata, frames, _, __, params):  # Removed unused 'time_info' and 'status'
    freq = params['freq']
    amp = params['amp']
    base = params['base']
    decay = params['decay']
    ...
    params['_phase'] = (phase + (freq * TABLE_SIZE / SAMPLE_RATE) * frames) % TABLE_SIZE  # Update phase in params